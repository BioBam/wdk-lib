...
One of the major benefits of *wdk*, is the ability to create high level abstractions ontop of the basic CWL building blocks. 
These abstractions, which we refer to as `L2` (Layer 2) constructs, are meant to simplify the interaction with cwl resources, and more accurately represent the mental model of the developer (author).
...
The process of hand-crafting these abstractions requires thourough research of use-cases and common patterns that emerge when authoring cwl tools and workflow applications.

At a certain point, when we've collected enough such use-cases, we will start consolidating it and build the first versions of these `L2` constructs.
...
- Aside from the `README`, the structure of the use-case directory is free form. It can range from proposing theoretical API's without any implementation, to a full blown *wdk* application that can be synthesized and deployed.

For example: [name](./sub-folder-name)

## Contribution

CWL has such a rich domain, and so many different configuration options, that come along with a lot of complexity for manifest authors. We feel that the best way to create these abstractions, is to engage with the community so that we understand the needs and the pain points. 
...
- While it makes sense to draw inspiration from API's suggested in existing use-cases, its important you have full creative freedom, so don't feel you have to be aligned with them.
